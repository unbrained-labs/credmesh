...
export function computePortfolio(state: AgentState): PortfolioReport {
  ...
  const totalDefaulted = rc(
    defaultedAdvances.reduce(
      (s, a) => s + a.approvedAmount - (a.repaidAmount ?? 0),
      0,
    ),
  );
  const totalBorrowed = rc(agents.reduce((s, a) => s + a.totalBorrowed, 0));

  const creditScores = agents.map((a) => computeCreditProfile(a).creditScore);
  ...
  const topBorrowers = agents
    ...
    .slice(0, 5)
    .map((a) => {
      const total = a.repaidAdvances + a.defaultedAdvances;
      return {
        address: a.address,
        name: a.name,
        totalBorrowed: a.totalBorrowed,
        outstandingBalance: a.outstandingBalance,
        creditScore: computeCreditProfile(a).creditScore,
        repaymentRate: total > 0 ? rc(a.repaidAdvances / total) : 1,
      };
    });
  ...
  return {
    summary: {
      ...
      repaymentRate:
        completedCount > 0
          ? rc(repaidAdvances.length / completedCount)
          : 1,
      defaultRate:
        completedCount > 0
          ? rc(defaultedAdvances.length / completedCount)
          : 0,
      averageAdvanceSize:
        advances.length > 0 ? rc(totalBorrowed / advances.length) : 0,
      averageCreditScore,
    },
    exposureByCategory,
    topBorrowers,
...
export function computeRisk(state: AgentState): RiskReport {
  ...
  // Utilization
  const totalLimits = agents.reduce(
    (s, a) => s + computeCreditProfile(a).creditLimit,
    0,
  );
  ...
  // Weighted default rate
  const totalAdvanced = rc(agents.reduce((s, a) => s + a.totalBorrowed, 0));
  const defaultedAdvances = advances.filter((a) => a.status === "defaulted");
  const defaultLossAmount = rc(
    defaultedAdvances.reduce(
      (s, a) => s + a.approvedAmount - (a.repaidAmount ?? 0),
      0,
    ),
  );
  const weightedDefaultRate =
    totalAdvanced > 0 ? rc(defaultLossAmount / totalAdvanced) : 0;
  ...
  const largestSingleExposure =
    agents.length > 0
      ? Math.max(...agents.map((a) => a.outstandingBalance))
      : 0;
  ...
  // Concentration risk (simplified HHI)
  ...
  // Alerts
  if (overdueCount > 0) {
    alerts.push(`${overdueCount} advance(s) are past due.`);
  }
  if (weightedDefaultRate > 0.1) {
    alerts.push(
      `Weighted default rate is ${(weightedDefaultRate * 100).toFixed(1)}%.`,
    );
  }
  if (concentrationRisk > 0.5) {
    alerts.push("Exposure is heavily concentrated in a single borrower.");
  }
  if (utilizationRate > 0.8) {
    alerts.push("Credit utilization exceeds 80%.");
  }
  if (state.treasury.availableFunds < totalOutstanding * 0.1) {
    alerts.push("Treasury reserves are below 10% of outstanding exposure.");
  }

  // Recommendations
  if (overdueCount > 0) {
    recommendations.push("Review overdue advances and consider default action.");
  }
  if (concentrationRisk > 0.3) {
    recommendations.push("Diversify lending across more agents.");
  }
  if (utilizationRate > 0.7) {
    recommendations.push("Increase treasury deposits to support growth.");
  }
  if (alerts.length === 0) {
    recommendations.push("Portfolio is healthy. Continue monitoring.");
  }
    ...
    agents: [
      {
        ...
        trustScore: 82,
        attestationCount: 15,
        cooperationSuccessCount: 8,
        successfulJobs: 12,
        failedJobs: 1,
        averageCompletedPayout: 95,
      },
      {
      ...
      },
      {
        ...
        trustScore: 68,
        attestationCount: 7,
        cooperationSuccessCount: 4,
        successfulJobs: 5,
        failedJobs: 1,
        averageCompletedPayout: 70,
      },
    ],
    jobs: [
      {
        agentAddress: "0xAgent_Atlas_Research_001",
        payer: "0xClient_AlphaDAO",
        title: "Market research on DeFi lending protocols",
        expectedPayout: 100,
        durationHours: 48,
        category: "research",
      },
      {
        agentAddress: "0xAgent_Codex_Builder_002",
        payer: "0xClient_BetaLabs",
        title: "Build smart contract integration module",
        expectedPayout: 150,
        durationHours: 72,
        category: "code",
      },
      {
        agentAddress: "0xAgent_Scout_Browser_003",
        payer: "0xClient_GammaCorp",
        title: "Competitive analysis web scraping pipeline",
        expectedPayout: 80,
        durationHours: 24,
    ...
    agents: [
      {
        ...
        trustScore: 45,
        attestationCount: 3,
        cooperationSuccessCount: 1,
        successfulJobs: 2,
        failedJobs: 2,
        averageCompletedPayout: 40,
      },
      {
        ...
        trustScore: 30,
        attestationCount: 0,
        cooperationSuccessCount: 0,
        successfulJobs: 0,
        failedJobs: 0,
        averageCompletedPayout: 0,
      },
    ],
    jobs: [
      {
        agentAddress: "0xAgent_Risky_Runner_004",
        payer: "0xClient_DeltaInc",
        title: "Automated onchain trade execution",
        expectedPayout: 60,
        durationHours: 12,
        category: "onchain",
      },
      {
        agentAddress: "0xAgent_New_Agent_005",
        payer: "0xClient_EpsilonDAO",
  ...
  return [
    {
      name: "happy",
      description:
        "Three agents with solid history complete jobs and repay advances in full. Credit profiles improve.",
      agents: 3,
      jobs: 3,
      advances: 3,
    ...
    },
    {
      name: "failure",
      description:
        "Two weak agents take advances. One delivers short, causing partial default. The other fails entirely.",
      agents: 2,
      jobs: 2,
      advances: 2,
    ...
    },
    {
      name: "both",
      description:
        "Runs both scenarios. Shows the contrast between strong and weak borrowers in the same portfolio.",
      agents: 5,
      jobs: 5,
      advances: 5,
      includesDefault: true,
